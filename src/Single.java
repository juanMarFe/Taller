...
public class Single {
	...
	public boolean ingresar(EmpresaComposite empresa) {
		...
		for (int i = 0; i < empresas.size(); i++) {
			if ((empresas.get(i).datos().startsWith("El NIT es: " + nit))
					&& (empresas.get(i).getContrasena().equals(contra))) {
				ingresar = true;
				break;
			} else {
				ingresar = false;
	...
	public void adicionarEmpresaAOtra(String nitBase, String nitAdicionar) {
		EmpresaComposite empresaBase = buscarEmpresa(nitBase);
		EmpresaComposite empresaAdicionar = buscarEmpresa(nitAdicionar);
		empresaBase.add(empresaAdicionar);
	}
